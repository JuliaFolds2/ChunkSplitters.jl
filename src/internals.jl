...
eltype(::ChunksIterator{T,C,Consecutive,ReturnIndices}) where {T,C} = UnitRange{Int}
eltype(::ChunksIterator{T,C,RoundRobin,ReturnIndices}) where {T,C} = StepRange{Int,Int}
eltype(c::ChunksIterator{T,C,Consecutive,ReturnViews}) where {T,C} = typeof(c[firstindex(c)])
eltype(c::ChunksIterator{T,C,RoundRobin,ReturnViews}) where {T,C} = typeof(c[firstindex(c)])
...
eltype(ec::Enumerate{<:ChunksIterator{T,C,Consecutive}}) where {T,C} = Tuple{Int,eltype(ec.itr)}
eltype(ec::Enumerate{<:ChunksIterator{T,C,RoundRobin}}) where {T,C} = Tuple{Int,eltype(ec.itr)}
